...
def transfer_style(vgg_model, decoder_model, content_img, style_img, interp_weights=None):
...
arg_parser.add_argument('--content_dir', type=str, help='Path to a directory of content images')
arg_parser.add_argument('--style', type=str, help='Path to the style image(s)')
arg_parser.add_argument('--style_dir', type=str, help='Path to a directory of style images')
...
# Additional options
arg_parser.add_argument('--content_size', type=int, default=512, help='Resize content image (set 0 to keep original size)')
arg_parser.add_argument('--style_size', type=int, default=512, help='Resize style image (set 0 to keep original size)')
arg_parser.add_argument('--crop', action='store_true', help='Apply center cropping')
arg_parser.add_argument('--save_ext', default='.jpg', help='Output image file extension')
arg_parser.add_argument('--output', type=str, default='output', help='Directory for saving output images')
...
decoder_model.load_state_dict(torch.load(args.decoder))
...
for content_file in content_files:
    for style_file in style_files:
        ...
        content_image = content_transform(Image.open(str(content_file))).to(device).unsqueeze(0)
        # Load and transform the style image
        style_image = style_transform(Image.open(str(style_file))).to(device).unsqueeze(0)
        with torch.no_grad():
            # Perform style transfer
            stylized_output = transfer_style(vgg_model, decoder_model, content_image, style_image)
        stylized_output = stylized_output.cpu()

        # Save the output stylized image
        output_filename = output_folder / f"{content_file.stem}_stylized_{style_file.stem}{args.save_ext}"
